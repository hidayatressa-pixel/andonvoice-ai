...
export const AlexaVoiceExperience: React.FC<AlexaVoiceExperienceProps> = ({ lines, calls, currentUser, language, onLanguageChange, onCreateCall }) => {
  ...
  return (
    <aside className="fixed bottom-5 right-5 z-50 flex h-[620px] max-h-[82vh] w-[420px] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-3xl border border-cyan-400/30 bg-slate-950 text-white shadow-2xl shadow-black/50">
      <header className="flex items-center justify-between border-b border-white/10 bg-gradient-to-r from-cyan-950 to-blue-950 px-5 py-4">
        <div className="flex items-center gap-3"><div className="rounded-xl bg-cyan-400/15 p-2"><Bot className="h-5 w-5 text-cyan-300" /></div><div><p className="font-black">AndonVoice AI</p><p className="text-xs text-cyan-200">{language === "id" ? "Simulasi pengalaman Alexa+" : "Alexa+ simulated experience"}</p></div></div>
      ...
      <div className="border-t border-white/10 p-4">
        <div className="mb-3 flex gap-2 overflow-x-auto pb-1">{(language === "id" ? INDONESIAN_EXAMPLES : ENGLISH_EXAMPLES).map((example) => <button key={example} onClick={() => respond(example)} className="whitespace-nowrap rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-[11px] text-slate-300 hover:border-cyan-400/50">{example}</button>)}</div>
