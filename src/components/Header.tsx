...
export const Header: React.FC<HeaderProps> = ({ activeTab, setActiveTab, activeCalls, soundConfig, setSoundConfig, currentUser, previewRole, onOpenLogin, onLogout, onOpenConfig, theme, setTheme, language, setLanguage }) => {
  ...
  const navClass=(tab:ActiveTab)=>`flex items-center gap-1.5 border px-3 py-1.5 rounded-xl font-bold transition-[background-color,color,border-color,box-shadow] whitespace-nowrap ${activeTab===tab?(isLight?"bg-white text-slate-900 shadow-sm border-slate-200":"bg-neutral-800 text-white shadow-sm border-neutral-700"):(isLight?"border-transparent text-slate-600 hover:text-slate-900 hover:bg-white/50":"border-transparent text-neutral-400 hover:text-neutral-200 hover:bg-neutral-900")}`;
